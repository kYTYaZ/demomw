        ...
        <article class="hentry post video">
            ...
            <div class="post__author author vcard inline-items">
                ...
                <div class="author-date">
                    <span   v-if="feed.attachment">
                        <a class="h6 post__author-name fn" :href="'/profile/'+user.profile.slug">{{user.name}}</a> compartió <a   :href="feed.attachment.url" v-if="feed.attachment.type==2"> un video</a>  <a   :href="feed.attachment.url" target="_blank"  v-if="feed.attachment.type==3"> un link</a>
                    </span>
            ...
            <div class="post-video" v-if="feed.attachment">
                <div class="video-thumb"  v-if="feed.attachment.image" >
                    <img  :src="feed.attachment.image" :alt="feed.attachment.title">
                    <a  :href="feed.attachment.url" target="_blank" v-if="feed.attachment.type==2" class="play-video">
                        <svg class="olymp-play-icon"><use xlink:href="/svg-icons/sprites/icons.svg#olymp-play-icon"></use></svg>
                    </a>

                    <a :href="feed.attachment.url" target="_blank" v-if="feed.attachment.type==3" class="play-video">
                        <i class="fas fa-link"   ></i>                                                    
                    </a>
                </div>

                <div class="video-content" v-if="feed.attachment">
                    <a :href="feed.attachment.url" class="h4 title" target="_blank">{{feed.attachment.title}}</a>
                    <p>{{feed.attachment.description}}
                    </p>
                    <a :href="feed.attachment.url" target="_blank" class="link-site">{{feed.attachment.site}}</a>
                </div>
            </div>
...
<script>
    export default {
        mounted() {
            ...
            console.log(this.user,"component video")
            this.feed.attachment = this.dataObject.attachment;
